...
public class PathFactory {
    ...
    private static final Object[] LOGICAL_PATH_MULTIMODE = new Object[]{PATH_ELEMENT_DEVICE, PATH_ELEMENT_MULTIMODE};
    private static final Object[] LOGICAL_PATH_MASTER = new Object[]{PATH_ELEMENT_DEVICE, PATH_ELEMENT_MASTER};
    ...
    */
    ...
    public static LogicalPath[] provideVoicePaths(ReadablePreset.ReadableVoice[] voices) {
        ...
        for (int i = 0; i < voices.length; i++)
            if (voices[i] instanceof ContextEditablePreset.EditableVoice && ((ContextEditablePreset.EditableVoice) voices[i]).getGroupMode())
                paths[i] = provideGroupPath(voices[i]);
            else
                paths[i] = providePresetPath(voices[i].getPreset()).append(new Object[]{PATH_ELEMENT_VOICES, voices[i].getVoiceNumber()});
        return paths;
    }
    ...
    public static LogicalPath[] provideVoiceSectionPaths(ReadablePreset.ReadableVoice[] voices, int sections) {
        ...
        for (int i = 0; i < voices.length; i++)
            if (voices[i] instanceof ContextEditablePreset.EditableVoice && ((ContextEditablePreset.EditableVoice) voices[i]).getGroupMode())
                paths[i] = provideGroupSectionPath(voices[i], sections);
            else
    ...
    public static LogicalPath[] provideLinkPaths(ReadablePreset.ReadableLink[] links) {
        ...
        for (int i = 0; i < links.length; i++)
            paths[i] = providePresetPath(links[i].getPreset()).append(new Object[]{PATH_ELEMENT_LINKS, links[i].getLinkNumber()});
        return paths;
    }
    ...
    public static LogicalPath[] provideZonePaths(ReadablePreset.ReadableVoice.ReadableZone[] zones) {
        ...
        for (int i = 0; i < zones.length; i++)
            paths[i] = provideVoicePath(zones[i].getVoice()).append(new Object[]{PATH_ELEMENT_ZONES, zones[i].getZoneNumber()});
        return paths;
    }
}
